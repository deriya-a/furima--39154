class User < ApplicationRecord
  ...
  validates :family_name_kanji, presence: true, format: { with: /\A[\p{Han}\p{Hiragana}\p{Katakana}ー－]+\z/, message: "漢字で入力してください" }
  validates :first_name_kanji, presence: true, format: { with: /\A[\p{Han}\p{Hiragana}\p{Katakana}ー－]+\z/, message: "漢字で入力してください" }
